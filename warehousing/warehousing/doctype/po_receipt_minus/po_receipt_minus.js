...
frappe.ui.form.on("PO Receipt Minus", {
    refresh(frm) {
        if (!frm.doc.eff_date){
    ...
    validate(frm){
        if (frm.doc.receipt_minus_item_serials.length === 0){
            frappe.msgprint({
                title: __('ERROR'),
                indicator: 'red',
        ...
        let fields = [
                ...
                { fieldtype: 'Section Break' },
                {
                    ...
                    fields: [
                        ...
                        {fieldname: "site", label: "Site", fieldtype: "Data", in_list_view: 1,  columns: 2, hidden: 1},
            ...
            const d = new frappe.ui.Dialog({
                ...
                primary_action(values) {
                    ...
                    for (let row of selected_rows) {
                        let match = frm.doc.receipt_minus_item_serials.find(item => item.po_line === row.poline && item.part_number === row.part && item.lot_serial === row.lotserial && item.current_location === row.location);
                        if (!match) {
                            let child = frm.add_child('receipt_minus_item_serials');
