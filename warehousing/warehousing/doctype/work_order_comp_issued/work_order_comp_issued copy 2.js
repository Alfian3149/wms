 	...
 	refresh(frm) {
        if (frm.doc.docstatus === 1){
         ...
         frm.add_custom_button(__('Entry Via Scanner'), function() {
            ...
            if (frm.doc.for_material_packaging__blending === "" || frm.doc.for_material_packaging__blending === undefined){
                frappe.msgprint("Please select first the proces for blending or packaging?");
                return;
            }
            else if (frm.doc.for_material_packaging__blending === "Packaging"){
                frappe.msgprint("Entry scanner can only active for Blending process");
                return;
            }
                ...
                if (scanned_items.length === 0) {
                    ...
                    for (let code in grouped) {
                        ...
                        group.details.forEach((item) => {
                            html += `
                                <tr>
                                    <td style="padding-left: 30px;">• Lot/Serial: ${item.lotserial} Qty Available: ${item.qty_lot_available}  </td>
