...
def create_inventory_record(site, part, lot_serial, reference, whs_location, initial_qty, invStatus, expireDate=None):
    ...
    if (conversion and conversion.um_packaging):
        new_inv.um_packaging = conversion.um_packaging 
    if (conversion and conversion.conversion_factor):
        new_inv.conversion_factor = conversion.conversion_factor
...
def get_fifo_picklist_with_reserved(itemPicklistName, item_status):
    ...
    for item in final_list:
        ...
        if total_qty_on_hand[0].qty == 0 :
            frappe.msgprint(f"There is not stock available for item : {item.part} .")
            continue
