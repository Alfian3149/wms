...
class WorkOrderValidator:
    ...
    def qty_tobe_produced(self, qty):
        ...
        fields=['sum(quantity_to_be_produced_immediately) as totalInputed', 'quantity_ordered', 'quantity_completed', 'um'])
        if getWO and getWO[0].totalInputed:
            totalQtyAllowed = flt(getWO[0].quantity_ordered) + flt(((getWO[0].quantity_ordered * 10) / 100))
            qtyAllowed = totalQtyAllowed - flt(getWO[0].totalInputed)
            if totalQtyAllowed < flt(qty) + flt(getWO[0].totalInputed): 
                frappe.throw(
